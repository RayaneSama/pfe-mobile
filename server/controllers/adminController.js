...
exports.viewGestionnaire = async (req, res) => {
  ...
  db.query(
    "SELECT gd.id_ge, gd.nom, gd.prenom, gd.date_naiss_gestio, c.email_co, c.photo_profil,eq.logo_eq,eq.nom_eq FROM `gestionnaire de club` AS gd JOIN compte AS c ON gd.id_co_ge = c.id_co JOIN equipe AS eq ON c.id_co = eq.id_co_ge_eq WHERE gd.id_ge = ?;",
    [GestionnaireId],
    (err, result) => {
      if (err) {
    ...
    (err, result) => {
      ...
      db.query(
        ...
        (err, pp) => {
          ...
          db.query(
            "SELECT logo_eq FROM equipe WHERE id_eq=?",
            [result[0].id_eq_std],
            (err, logo) => {
              if (err) {
...
exports.gererStades = async (req, res) => {
  ...
  db.query(
    "SELECT id_std,nom_std,ville_std,nom_eq,logo_eq FROM stade JOIN equipe ON equipe.id_eq = stade.id_eq_std;",
    (err, result) => {
      if (err) {
...
exports.viewMatches = async (req, res) => {
  ...
  db.query("select * from `match` where id_ma=?", [MatchId], (err, match) => {
    ...
    db.query(
      "SELECT nom_ar,prenom_ar FROM arbitre WHERE id_ar=? ",
      [match[0].id_ar_ma],
      (err, arbitre) => {
        if (err) {
        ...
        db.query(
          ...
          (err, pp) => {
            ...
            db.query(
              "SELECT nom_std,ville_std FROM stade JOIN `match` ON match.id_std_ma = stade.id_std WHERE id_ma = ?",
              [MatchId],
              (err, stade) => {
                if (err) {
                ...
                db.query(
                  "SELECT nom,prenom FROM `gestionnaire de club` JOIN `match` ON match.id_ge_ma= `gestionnaire de club`.id_ge WHERE id_ma = ?",
                  [MatchId],
                  (err, gest) => {
                    if (err) {
      ...
      (err, pp) => {
        ...
        db.query(
          ...
          (err, results) => {
            ...
            db.query(
              "SELECT equipe_1,equipe_2,date_ma,horaire_ma,id_std_ma,id_ar_ma,journee_ma FROM `match` WHERE id_ma=?",
              [MatchId],
              (err, equipe) => {
                if (err) {
                ...
                db.query(
                  "SELECT nom, prenom, id_ge FROM `gestionnaire de club` JOIN `match` ON `match`.id_ge_ma = `gestionnaire de club`.id_ge WHERE id_ma = ? ",
                  [MatchId],
                  (err, gest) => {
                    if (err) {
                      console.log(
                        "avoir erreur data gestionnaire du match " + err
                      );
                    }
                    db.query(
                      "SELECT id_std,nom_std,ville_std FROM stade",
                      (err, stade) => {
                        if (err) {
                          console.log("erreur sql avoir stades" + err);
                        }
                        db.query(
                          "SELECT id_ar,nom_ar,prenom_ar FROM arbitre",
                          (err, arbitre) => {
                            if (err) {
                            ...
                            res.render(
                              "../views/Admin/Matches/modifierMatches",
                              {
                                result,
                                stade,
                                results,
